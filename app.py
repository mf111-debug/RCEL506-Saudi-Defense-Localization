...
COUNTRY_CONFIG = {
    ...
    "Türkiye": {
        "file_prefix": "TradeData_TK_",
        "sipri_row": 12,
        "color": "#E05252",
        "partner_filter": "rkiye",
    },
    "Rep. of Korea": {
        "file_prefix": "TradeData_TK_",
        "sipri_row": 9,
        "color": "#4A9FD5",
        "partner_filter": "Korea",
    }
}
...
def load_arms_data(country_name, start_year=2010, end_year=2024):
    ...
    for year in range(start_year, end_year + 1):
        ...
        possible_names = [
            f"{config['file_prefix']}{year}.csv",
            f"TradeData_TK_{year}.csv",
            f"TradeData_T_K_{year}.csv",
        ]
        raw = None
        ...
        try:
            ...
            if config['partner_filter']:
                # partnerISO has country codes, use it for filtering
                if 'partnerISO' in df.columns:
                    df['partnerISO'] = df['partnerISO'].astype(str)
                    df = df[df['partnerISO'].str.contains(config['partner_filter'], na=False, case=False)]
